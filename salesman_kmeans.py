...
if __name__=='__main__':
    if len(sys.argv)< 3:
        print 'Usage: cluster dataset_path k maxIter batch'
        exit(1)
    fname= sys.argv[1]
    if len(sys.argv) ==4:
        maxIter= sys.argv[3]
        ...
        for i in range(k):
            if len(cluster[i])>0:
                for j in range(len(cluster[i][0])):
                    # for each gene data
                    gene= [ l[j] for l in cluster[i]]  
                    minDist= float('inf')
                    minCluster= 0           
                    for pi, p in enumerate(path):
                        dist= calDistance (p, gene)
                        if dist< minDist:
                            minDist= dist
                            minCluster= pi
                    if minCluster!= i:
                        change= True
                        [cluster[minCluster][j].append( gene[j]) for j in len(gene) ]       #update cluster    
                        path[minCluster]= salesman(cluster[minCluster])
                        [del cluster[i][j] for ind in range(len(gene))]
                        path[i]= salesman[cluster[i]]
        if change== False:
            break
        iloop= iloop+1
